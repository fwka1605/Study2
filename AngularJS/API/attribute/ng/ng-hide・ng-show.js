<element ng-show="ブール値"></element>
<element ng-hide="ブール値"></element>
	
ブール値に応じて、現在の要素の表示非表示を切り替えます。
ng-showとng-hideの動作は同じであり、ただブール値と表示・非表示の
関係が逆になっているだけである。
	
ng-ifで非表示にした際には、DOMツリーから廃棄されていたが、
こちらは、DOMツリーから破棄されるのではなく、ブール値がtruの時は
class属性にng-showまたはng-hideが指定され、AngularJSのCSSにより
elementが表示・非表示になるだけである。
...
<div id="panel" class="panel panel-default" ng-show="show">
...
ng-showにshowを指定し、そのshowをチェックボックスとモデルで紐づけています。
