...
public class TableDefineUtilForSqlServer {
	...
	private static void getColumnAttributes(ColumnDto columnDto,String tableName,String columnName){
		...
		try {
			...
			preStm.setString(1, tableName.toUpperCase());
			preStm.setString(2, columnName.toUpperCase());
			...
			if (rs.next()) {
				...
				int pk = rs.getInt("pk_constraint_type");
				int uq = rs.getInt("uq_constraint_type");
				if(pk==1){
					columnDto.setConstraintType("pk");
					columnDto.setConstraintPosition(rs.getInt("pk_position"));
					columnDto.setComments("");
				}
				else if(uq==1){
					columnDto.setConstraintType("uq");
					columnDto.setConstraintPosition(rs.getInt("uq_position"));
					columnDto.setComments("");
				}
	...
	private static void getIndexList(TableDto tableDto){
		...
		try {
			...
			preStm = connectionManagerUtil.getConnection().prepareStatement(
					...
					"		ON ind.id=obj.id and obj.xtype='U'	" + "\n" +
					"	where									" + "\n" +
					"			obj.name= ?						" + "\n" +
			...
			for(IndexDto dtoTmp:tableDto.getIndexList()){
				...
				preStm = connectionManagerUtil.getConnection().prepareStatement(
						"	SELECT															" + "\n" +
								...
								"		,ind_key.keyno AS column_position							" + "\n" +
								"	FROM															" + "\n" +
								"		sysindexkeys ind_key										" + "\n" +
								"		INNER JOIN sysobjects obj									" + "\n" +
								"			ON ind_key.id = obj.id AND obj.xtype = 'U'				" + "\n" +
								"		INNER JOIN syscolumns col									" + "\n" +
								"			ON ind_key.id = col.id AND ind_key.colid = col.colid	" + "\n" +
								"		INNER JOIN sysindexes ind									" + "\n" +
								"			ON ind_key.id = ind.id AND ind_key.indid = ind.indid	" + "\n" +
								"	WHERE															" + "\n" +
								"			Upper(obj.name) = ?										" + "\n" +
								"		And Upper(ind.name) = ?										" + "\n" +
								"	ORDER BY														" + "\n" +
				...
				preStm.setString(1, dtoTmp.getTableName().toUpperCase());
				preStm.setString(2, dtoTmp.getIndexName().toUpperCase());
